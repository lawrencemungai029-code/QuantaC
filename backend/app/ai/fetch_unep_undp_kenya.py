...
async def fetch_unep_undp_kenya():
    url = "https://www.ke.undp.org/content/kenya/en/home/projects.html"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            html = await resp.text()
            ai_logger.info(f"Fetched UNDP Kenya projects at {datetime.utcnow().isoformat()}")
            return [{
                "source": "undp",
                "url": url,
